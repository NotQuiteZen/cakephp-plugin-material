 ...
 * Class FormHelper
 *
 * Form helper for http://daemonite.github.io/material/
 *
...
class FormHelper extends \Cake\View\Helper\FormHelper {
    ...
    public function create($model = null, array $options = []) {
        ...
        if ($this->isLayout('inline')) {
            $options = $this->customAddClass($options, 'form-inline');
            $this->_setTemplatesInternal([
                'inputContainer' => '{{content}}{{help}}']);
        } else {
            if ($this->isLayout('grid')) {
                $options = $this->customAddClass($options, ['container']);
    ...
    public function fieldset($fields = '', array $options = []) {
        if ( ! isset($options['fieldset']) || $options['fieldset'] !== false) {
            $options = $this->customAddClass($options, 'form-group', ['useIndex' => 'fieldset.class']);
        }
    ...
    protected function _addWidgetClass($options = [], $widgetType = null) {
        ...
        if ($type !== 'hidden' && $class !== null) {
            $options = $this->customAddClass($options, $this->getConfig('layout.classes.control'));
        }
        $options = $this->customAddClass($options, $class);
                                ...
                                if (isset($item['text'])) {
                                    ...
                                    switch ($item['type']) {
                                        ...
                                        case 'btn':
                                            ...
                                            break;
                                        default:
                                            $item = $this->customAddClass($item, 'input-group-text');
    ...
    private function renderPrependAppend($input, $options) {
        ...
        $attrs = $this->customAddClass($attrs, 'input-group');

        switch ($size) {
            case 'large':
            case 'lg':
                $attrs = $this->customAddClass($attrs, 'input-group-lg');
                break;
            case 'small':
            case 'sm':
                $attrs = $this->customAddClass($attrs, 'input-group-sm');
                break;
        }
    ...
    private function formatHelp(&$options) {
        ...
        if ($options['templateVars']['help']) {
            ...
            if (is_string($help)) {
                ...
                if (is_array($help) && isset($help['text'])) {
            ...
            if (isset($data['content'])) {
                // Add the class
                $data['attrs'] = $this->customAddClass($data['attrs'], ['form-text', 'text-muted']);
    ...
    private function switchTemplates(&$options, $type = null) {
        ...
        if ($options['customControls']) {
            ...
            switch ($type) {
                case 'checkbox':
                    $newTemplates = [
                        'checkboxContainer' => '<div class="form-group"><div class="custom-control custom-checkbox"{{attrs}}>{{content}}{{error}}{{help}}</div></div>',
                    ];
                    break;
                case 'multicheckbox':
                    $newTemplates = [
                        'checkboxWrapper' => '<div class="form-group"><div class="custom-control custom-checkbox">{{label}}</div></div>',
    ...
    private function setLabelClass(&$options, $type = null, $fromInput = false) {
        ...
        if ($type === 'file') {
            ...
            if ($this->isLayout('grid')) {
                $label = $this->customAddClass($label, $options['gridClasses'][0], ['useIndex' => false]);
            }
        }

        // If there is a label
        if ($label !== false) {
            $label = $this->customAddClass($label, $this->getConfig('layout.classes.label'), ['useIndex' => false]);
        }
    ...
    private function parseButtonClass(&$options) {
        ...
        $options = $this->customAddClass($options, $newClasses);
    ...
    protected function _datetimeOptions($options) {
        ...
        foreach ($this->_datetimeParts as $type) {
            if (is_array($options[$type])) {
                $options[$type] = $this->customAddClass($options[$type], 'form-control');
            }
        }
    ...
    private function formatDateTimes(&$options) {
        ...
        if (is_object($options['val'])) {
            ...
            if (get_class($options['val']) === 'Cake\Chronos\Chronos') {
                ...
                if (in_array(get_class($options['val']), $i18nFormatClasses)) {
                    switch ($options['type']) {
                        case 'date':
                            $format = "yyyy-MM-dd";
                            break;
